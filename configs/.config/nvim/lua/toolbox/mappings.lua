local M = {}
---https://github.com/toolboxsojka/dotfiles2/blob/master/.vim/lua/toolboxvim/utils/mappings.lua
  ...
  if type(action) == 'function' then
    if is_buffer then
      ...
      if (mode == 'v' or mode == 'x') then
        action = ([[:<C-u>lua toolbox.mappings._LUA_BUFFER_MAPPINGS[%d]['%s']()<CR>]]):format(bufnr, escaped_key)
      else
        action = ([[<Cmd>lua toolbox.mappings._LUA_BUFFER_MAPPINGS[%d]['%s']()<CR>]]):format(bufnr, escaped_key)
      end
    else
      M._LUA_MAPPINGS[escaped_key] = action

      if (mode == 'v' or mode == 'x') then
        action = ([[:<C-u>lua toolbox.mappings._LUA_MAPPINGS['%s']()<CR>]]):format(escaped_key)
      else
        action = ([[<Cmd>lua toolbox.mappings._LUA_MAPPINGS['%s']()<CR>]]):format(escaped_key)
      end
    end
...
function M.register_mappings(mappings, default_options, which_key_dict)
  for keys, mapping in pairs(mappngs) do
...
--   -- name = name or ('Lua_autocmd_' .. toolbox.utils.unique_id())

--   -- if type(def[#def]) == 'function' then
--   --   local fn = def[#def]

--   --   def = {unpack(def, 1, #def - 1)}
--   --   table.insert(def, ("lua toolbox.mappings._LUA_AUGROUP_HOOKS['%s']()"):format(name))
...
--   --     toolbox.command('augroup LuaAugroup_' .. group_name)
--   --     toolbox.command('autocmd!')

--   --     for index,def in ipairs(defs) do
--   --       M.create_autocmd(def, group_name .. index)
--   --     end

--   --     toolbox.command('augroup END')
--   --   end
--   -- end
-- end

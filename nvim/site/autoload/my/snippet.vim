...
"    echomsg empty(b:projectionist)
"    echomsg exists('g:did_plugin_ultisnips')

  if !exists('g:did_plugin_ultisnips') || !(line('$') == 1 && getline('$') == '') || filereadable(expand('%:p'))
    " echomsg ' - Abort on non-empty buffer or extant file'
    return
  endif

  if !empty(b:projectionist)
    " Loop through projections
    " with 'skeleton' key and try each one until the
    " snippet expands
  ...
  echomsg 'Try generic _skel template as last resort'
